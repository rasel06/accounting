<x-helpers.parts.content-panel>
    ...
    <x-helpers.parts.data-table.control :showStatus="false">
        ...
        <x-helpers.parts.date-range name="asset" />

    </x-helpers.parts.data-table.control>

    <x-helpers.parts.data-table.assets-table :tableItems="$debitTransactionList" :$limitFilter :$tableFields />

    <x-slot name="modal">
        <x-helpers.modal :id>
            <x-helpers.forms.assets :$accountList :selectedAccountId="$accountId" :$storeList :selectedStoreId="$storeId" :$assetTypeList
